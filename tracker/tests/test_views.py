...
def test_total_values_appear_on_list_page(user_transactions, client):
    ...
    income_total = sum(t.amount for t in user_transactions if t.type == 'receita')
    expense_total = sum(t.amount for t in user_transactions if t.type == 'despesa')
...
@pytest.mark.django_db
def test_transatcion_type_filter(user_transactions, client):
    user = user_transactions[0].user
    client.force_login(user)

    #receita check
    GET_params = {'transaction_type': 'receita'}
    response = client.get(reverse('transactions-list'), GET_params)
    qs = response.context['filter'].qs 

    for transaction in qs:
        assert transaction.type == 'receita'

    #despesa check
    GET_params = {'transaction_type': 'despesa'}
    response = client.get(reverse('transactions-list'), GET_params)
    qs = response.context['filter'].qs 
...
def test_category_filter(user_transactions, client):
    ...
    #pegar duas primeiras categorias - chave primária
...
def test_update_transaction_request(user, transaction_dict_params, client):
    ...
    # update the transaction via POST request - mutate the dict params
